...
- ✓ User's alias is shown in the UI
...
**Behavior:**
- ✅ **Pass**: Screenshot hash matches expected → Test passes
- ❌ **Fail**: Screenshot hash differs → Test fails with clear error message
- 📸 **Create**: Screenshot doesn't exist → Creates it (first run only)
...
### ❌ PROHIBITED: waitForTimeout

```typescript
// ❌ ILLEGAL - Never use arbitrary timeouts
await page.waitForTimeout(1000);
await page.waitForTimeout(500);

// ❌ ILLEGAL - Even short timeouts are not allowed
...
### ✅ ALLOWED: Single Animation Frame

When you need to wait for a single render cycle:

```typescript
// ✅ Wait for one animation frame (typically 16ms at 60fps)
await page.evaluate(() => new Promise(requestAnimationFrame));

// ✅ Wait for two animation frames (for double-buffered rendering)
...
1. ✅ **Include e2e tests** that demonstrate the UI change
2. ✅ **Regenerate screenshots** for affected tests
3. ✅ **Update test README.md** files if verification steps change
4. ✅ **Document new visual elements** in screenshot descriptions
5. ✅ **Verify pixel-perfect rendering** before committing
...
2. ✅ **Verify state and DOM programmatically** - don't rely on screenshots for bug detection
3. ✅ **Generate screenshots automatically** at each significant step
4. ✅ **Create README.md files** documenting what was verified
5. ✅ **Use pixel-perfect screenshot comparison** to catch visual regressions
6. ✅ **Never use waitForTimeout** - use event-based waiting
7. ✅ **Wait for animation frames** when needed for rendering
8. ✅ **Update screenshots** when UI intentionally changes
9. ✅ **Use helpers** to avoid code duplication
10. ✅ **Clean up** browser contexts in multi-user tests
