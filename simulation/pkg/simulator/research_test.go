...
func TestInitializeTechnologyResearch(t *testing.T) {
	...
	expectedIndependentCost := StoneKnappingScienceRequired - FireMasteryScienceRequired // 50 points
...
func TestAddResearchPoints(t *testing.T) {
	...
	state.SetResearchFocus("Stone Knapping")
	err = state.AddResearchPoints(20.0) // 30 + 20 = 50 (independent cost)
	if err != nil {
		t.Fatalf("Failed to add research points: %v", err)
	}
	
	stoneTech, _ = state.GetTechnologyProgress("Stone Knapping")
	if stoneTech.ProgressPoints != 50.0 {
		t.Errorf("Expected 50 progress points, got %f", stoneTech.ProgressPoints)
	}
	if !stoneTech.IsUnlocked {
		t.Error("Stone Knapping should be unlocked at 50 points")
	}
...
func TestIndependentResearchCosts(t *testing.T) {
	...
	state.SetResearchFocus("Stone Knapping")
	state.AddResearchPoints(50.0) // Complete Stone Knapping first!
